        ...
        try:
            ...
            def process_batch(batch_index, batch):
                try:
                    ...
                    batch_input_tokens = sum(self.estimate_tokens(str(v)) for v in categorize_input.values())
                    ...
                    for item in batch_results['cat']:
                        ...
                        valid_batch_results.append({
                            'ticket_id': item['id'],
                            'categorias': [cat.strip() for cat in item['cat'] if isinstance(cat, str)][:3]
                        })
                    ...
                    # Salva log de erro
                    log_file = self.database_dir / f"error_log_batch_{batch_index}_{int(time.time())}.json"
                    ...
                    error_data = {
                        ...
                        "extracted_json": json_str if 'json_str' in locals() else "No JSON"
                    }
                    ...
                    return {
                        'results': [],
                        'input_tokens': batch_input_tokens if 'batch_input_tokens' in locals() else 0,
                        'output_tokens': batch_output_tokens if 'batch_output_tokens' in locals() else 0,
                        'success': False,
                        'error': str(e)
                    }
                ...
                for future in tqdm(concurrent.futures.as_completed(future_to_batch), 
                    ...
                    try:
                        ...
                        if result['success']:
                            ...
                            print(f"Batch {batch_index}/{len(ticket_batches)} processado: {len(result['results'])} tickets categorizados")
                        else:
