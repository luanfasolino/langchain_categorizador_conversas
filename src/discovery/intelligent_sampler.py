...
class IntelligentSampler:
    ...
    def __init__(self, strategy: str = "stratified", random_state: int = 42):
        ...
        if strategy not in ["stratified", "diversity", "hybrid"]:
            raise ValueError(
                f"Invalid strategy: {strategy}. Must be one of: stratified, diversity, hybrid"
            )
    ...
    def sample_tickets(
        ...
        if max_tickets < min_tickets:
            raise ValueError(
                f"max_tickets ({max_tickets}) must be greater than or equal to min_tickets ({min_tickets})"
            )
        else:
        ...
        self.logger.info(
            f"Sampling {target_size} tickets from {total_tickets} using {self.strategy} strategy"
        )
        ...
        for month, count in month_counts.items():
            ...
            if len(month_data) <= month_sample_size:
                ...
                sampled_month = month_data.sample(
                    n=month_sample_size, random_state=self.random_state
                )
        ...
        if len(result) > target_size:
            ...
            if len(excluded) > 0:
                additional = excluded.sample(
                    n=min(remaining, len(excluded)), random_state=self.random_state
                )
    ...
    def validate_sample_representativeness(
        ...
        if common_months:
            ...
            kl_divergence = sum(orig_dist * np.log(orig_dist / samp_dist))
        ...
        if common_senders:
            sender_diff = sum(
                abs(orig_senders[s] - sample_senders.get(s, 0)) for s in common_senders
            )
        ...
        metrics["unique_tickets_ratio"] = (
            sample_df["ticket_id"].nunique() / original_df["ticket_id"].nunique()
        )
...
def create_diversity_report(
    sampler: IntelligentSampler, original_df: pd.DataFrame, sample_df: pd.DataFrame
) -> str:
    """
