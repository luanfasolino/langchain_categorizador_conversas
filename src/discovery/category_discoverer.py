...
class CategoryDiscoverer(BaseProcessor):
    ...
    def __init__(
        ...
        self.logger.info(
            f"CategoryDiscoverer initialized with {model_name}, "
...
"""
    ...
    def discover_categories(
        ...
        try:
            ...
            self.logger.info(
                f"Category discovery completed: {categories['discovery_stats']['categories_created']} categories found"
            )
            return categories
    ...
    def _extract_categories(self, consolidated_analysis: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    "Could not parse categories response as valid JSON"
                ) from e
        except Exception as e:
    ...
    def _prepare_tickets_text(self, tickets_df: pd.DataFrame) -> str:
        ...
        combined_text = "\n".join(ticket_texts)
        self.logger.info(
            f"Prepared text for {len(ticket_texts)} tickets, {len(combined_text)} characters"
        )
    ...
    def _validate_and_enhance_categories(
        self, categories: Dict[str, Any], tickets_df: pd.DataFrame
    ) -> Dict[str, Any]:
        """
        ...
        if len(category_list) < self.min_categories:
            self.logger.warning(
                f"Only {len(category_list)} categories found, minimum is {self.min_categories}"
            )
        elif len(category_list) > self.max_categories:
            self.logger.warning(
                f"{len(category_list)} categories found, maximum is {self.max_categories}"
            )
    ...
    def load_categories(self, categories_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(categories.get('categories', []))} categories from {categories_path}"
            )
            return categories
    ...
    def get_discovery_stats(self, categories: Dict[str, Any]) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "avg_examples_per_category": np.mean(
                [len(cat.get("examples", [])) for cat in category_list]
            ),
