...
class PerformanceMonitor:
    ...
    def __init__(self,
        ...
        self.logger.info(f"PerformanceMonitor initialized with targets: "
    ...
    def validate_pipeline_results(self,
                                 ...
                                 results_path: Path) -> ValidationReport:
        """
    ...
    def _calculate_accuracy_metrics(self, 
                                  ...
                                  orchestration_metrics) -> AccuracyMetrics:
        ...
        category_coverage = len(used_category_ids) / total_categories if total_categories > 0 else 0.0
        
        # Consistency score (based on confidence distribution)
        confidence_std = confidences.std()
        consistency_score = max(0, 1 - (confidence_std / avg_confidence)) if avg_confidence > 0 else 0
        ...
        if avg_confidence < self.confidence_target:
            validation_errors.append(f"Average confidence {avg_confidence:.3f} below target {self.confidence_target:.3f}")
    ...
    def _generate_recommendations(self,
                                cost_metrics: CostMetrics,
                                performance_metrics: PerformanceMetrics,
                                accuracy_metrics: AccuracyMetrics) -> List[str]:
        ...
        if not performance_metrics.meets_time_target:
            time_overage = (performance_metrics.total_processing_time / 60) - performance_metrics.target_time_minutes
            recommendations.append(
    ...
    def generate_performance_dashboard(self,
        ...
        if output_path is None:
            output_path = self.monitoring_dir / f"dashboard_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        ...
        timestamps = [report.timestamp for report in validation_reports]
        costs_per_1k = [report.cost_metrics.cost_per_1k_tickets for report in validation_reports]
        processing_times = [report.performance_metrics.total_processing_time / 60 for report in validation_reports]
        confidences = [report.accuracy_metrics.avg_confidence for report in validation_reports]
        classification_rates = [report.accuracy_metrics.classification_rate for report in validation_reports]
        
        # Plot 1: Cost Tracking
        ax1.plot(range(len(costs_per_1k)), costs_per_1k, 'b-o', label='Actual Cost')
        ax1.axhline(y=self.cost_target_per_1k, color='r', linestyle='--', label=f'Target (${self.cost_target_per_1k:.2f})')
        ...
        ax2.axhline(y=self.time_target_minutes, color='r', linestyle='--', label=f'Target ({self.time_target_minutes}min)')
        ...
        ax3.axhline(y=self.confidence_target, color='r', linestyle='--', label=f'Target ({self.confidence_target:.2f})')
        ...
        ax4.plot(range(len(classification_rates)), classification_rates, 'c-o', label='Classification Rate')
    ...
    def compare_against_baseline(self,
                               current_metrics,
                               baseline_metrics,
                               comparison_name: str = "Current vs Baseline") -> Dict[str, Any]:
        ...
        comparison_results['summary'] = {
            ...
            'overall_better': len(comparison_results['improvements']) > len(comparison_results['regressions'])
        }
    ...
    def run_ab_test_simulation(self,
                              ...
                              test_name: str = "A/B Test") -> Dict[str, Any]:
        ...
        ab_test_report = {
            ...
            'recommendation': self._generate_ab_recommendation(metrics_a, metrics_b, statistical_results)
        }
        
        # Save A/B test report
        ab_report_path = self.monitoring_dir / f"ab_test_{test_name.lower().replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _calculate_ab_test_metrics(self, results_df: pd.DataFrame, method_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'unique_categories_used': len(set([
                int(x) for cat_ids in results_df['category_ids'].dropna() 
                if cat_ids for x in cat_ids.split(',')
            ]))
        }
    
    def _perform_statistical_comparison(self,
                                      results_a: pd.DataFrame,
                                      results_b: pd.DataFrame) -> Dict[str, Any]:
        ...
        pooled_std = np.sqrt(((len(conf_a) - 1) * conf_a.var() + (len(conf_b) - 1) * conf_b.var()) / (len(conf_a) + len(conf_b) - 2))
    ...
    def _generate_ab_recommendation(self,
        ...
        a_better_classification = metrics_a['classification_rate'] > metrics_b['classification_rate']
        
        if a_better_confidence and a_better_classification:
            return f"Recommend Method A: significantly better confidence and classification rate"
        elif not a_better_confidence and not a_better_classification:
            return f"Recommend Method B: significantly better confidence and classification rate"
        else:
            return ("Mixed results: consider business priorities. "
...
def create_performance_monitor(monitoring_dir: str,
                             cost_target: float = 0.20,
                             time_target: float = 25,
                             confidence_target: float = 0.85) -> PerformanceMonitor:
...
def validate_opçao_d_compliance(orchestration_metrics,
                               categories_path: str,
                               results_path: str,
                               monitoring_dir: str = "database/monitoring") -> ValidationReport:
    """
...
if __name__ == "__main__":
    ...
    monitor.start_monitoring_session("test_session", {
        'total_tickets': 1000,
