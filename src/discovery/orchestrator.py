...
class TwoPhaseOrchestrator(BaseProcessor):
    ...
    def execute_complete_pipeline(self,
                                 ...
                                 force_reclassification: bool = False) -> OrchestrationMetrics:
        """
    ...
    def _execute_discovery_phase(self,
        ...
        try:
            ...
            self.logger.info(f"Sampling {self.config.sample_rate:.1%} of tickets using {self.config.sampling_strategy} strategy")
            ...
            self.logger.info(f"Discovered {len(discovered_categories.get('categories', []))} categories")
    ...
    def _execute_application_phase(self,
                                  tickets_df: pd.DataFrame,
                                  categories_path: Path,
                                  output_dir: Path,
                                  force_reclassification: bool) -> Path:
        ...
        try:
            ...
            categories = self.classifier.load_categories(categories_path)
    ...
    def _generate_final_metrics(self,
                               ...
                               categories_path: Path,
                               results_path: Path,
                               output_dir: Path) -> OrchestrationMetrics:
    ...
    def _log_final_summary(self, metrics: OrchestrationMetrics):
        ...
        self.logger.info(f"{cost_status} Cost target: ${metrics.cost_per_1k_tickets:.4f} "
                        f"{'<=' if metrics.meets_cost_target else '>'} ${self.config.cost_target_per_1k:.2f}")
        self.logger.info(f"{confidence_status} Confidence target: {metrics.avg_confidence:.3f} "
...
def run_complete_pipeline(input_file: str,
                         output_dir: str,
                         api_key: str,
                         config: Optional[Dict[str, Any]] = None) -> OrchestrationMetrics:
    """
