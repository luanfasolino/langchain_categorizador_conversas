...
class TicketSummarizer(BaseProcessor):
    ...
    def process_tickets(self, input_file: Path, nrows: int = None) -> Path:
        ...
        try:
            ...
            print(
                f"Tokens na fase Bullet - Input: {bullet_tokens_in:,}, Output: {bullet_tokens_out:,}"
            )
            ...
            for item in bullets:
                if not isinstance(item, dict) or "bullet" not in item:
                    continue
                valid_bullets.append({"bullet": item["bullet"].strip()})

            if len(valid_bullets) != 15:
                print(
                    f"Aviso: Número de bullet points ({len(valid_bullets)}) diferente do esperado (15)"
                )
