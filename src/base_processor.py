...
class BaseProcessor:
    def __init__(self, api_key: str, database_dir: Path, max_tickets_per_batch: int = 50, max_workers: int = None, use_cache: bool = True):
        self.database_dir = database_dir
        self.max_tickets_per_batch = max_tickets_per_batch
        self.total_input_tokens = 0
        self.total_output_tokens = 0
        self.max_workers = max_workers or min(os.cpu_count(), 4)  # Limita ao número de CPUs ou 4, o que for menor
    ...
    def process_batches(self, chain, tickets: list, token_limit: int = 50000) -> Tuple[list, int, int]:
        ...
        def process_batch(tickets_text, retry_count=0):
            ...
            try:
                ...
                print(f"Tokens estimados neste batch - Input: {input_tokens:,}, Output: {output_tokens:,}")
                ...
                try:
                    ...
                    tickets_no_batch = len(tickets_text.split("Ticket ")) - 1  # -1 para compensar o header
                    ...
                    if len(valid_results) != len(batch_result):
                        print(f"Aviso: {len(batch_result) - len(valid_results)} resultados foram filtrados por serem inválidos")
    ...
    def prepare_data(self, input_file: Path, nrows: int = None) -> List[dict]:
        ...
        agent_messages = df_filtered[df_filtered['sender'].isin(['AGENT', 'HELPDESK_INTEGRATION'])]['ticket_id'].value_counts()
        ...
        print(f"Tickets com pelo menos duas mensagens do USER e duas do AGENT: {len(valid_tickets)}")
        
        # Agrupa os textos por ticket_id
        grouped = df_filtered.groupby("ticket_id").agg({
    ...
    def process_batches_parallel(self, chain, tickets: list, token_limit: int = 50000) -> Tuple[list, int, int, int]:
        ...
        for ticket in tickets:
            ...
            if batch_tokens + ticket_tokens >= token_limit or len(batch_tickets) >= self.max_tickets_per_batch:
        ...
        def process_single_batch(batch_index, batch):
            try:
                ...
                # Log do erro
                log_file = self.database_dir / f"error_log_batch_{batch_index}_{int(time.time())}.log"
                with open(log_file, "w", encoding="utf-8") as f:
            ...
            for future in tqdm(concurrent.futures.as_completed(future_to_batch), 
                ...
                try:
                    ...
                    if result['success']:
                        ...
                        print(f"Batch {batch_index} processado com sucesso: {len(result['results'])} resultados")
                    else:
                        print(f"Falha no batch {batch_index}: {result.get('error', 'Erro desconhecido')}")
                except Exception as e:
    ...
    def process_chunks_with_cache(self, chunks: List[Document], map_chain, processor_name: str) -> Tuple[List[str], int, int]:
            ...
            for future in tqdm(concurrent.futures.as_completed(future_to_chunk), 
                ...
                try:
                    ...
                    if result['success']:
                        ...
                        total_input_tokens += result['input_tokens']
                        total_output_tokens += result['output_tokens']
                        print(f"Chunk {chunk_index}/{len(chunks)} processado - Input: {result['input_tokens']:,}, Output: {result['output_tokens']:,}")
                    else:
    ...
    def process_batches_with_cache(self, batches: List[List[dict]], process_func, processor_name: str) -> Tuple[List[dict], int, int]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Submete todos os batches para processamento
            future_to_batch = {
                executor.submit(self._process_batch_with_cache, i, batch, process_func, processor_name): (i, batch) 
            ...
            for future in tqdm(concurrent.futures.as_completed(future_to_batch), 
                ...
                try:
                    ...
                    if result['success']:
                        ...
                        total_input_tokens += result['input_tokens']
                        total_output_tokens += result['output_tokens']
                        print(f"Batch {batch_index}/{len(batches)} processado: {len(result['results'])} itens")
                    else:
                        print(f"Falha no batch {batch_index}: {result.get('error', 'Erro desconhecido')}")
                except Exception as e:
                    print(f"Exceção ao processar batch {batch_index}: {str(e)}")
    ...
    def _process_batch_with_cache(self, batch_index: int, batch: List[dict], process_func, processor_name: str) -> Dict[str, Any]:
        ...
        cache_key = self._generate_cache_key({
            ...
            'method': f'{processor_name}_batch'
        })
