...
class TicketReportGenerator(BaseProcessor):
    ...
    def analyze_categorization_results(
        ...
        available_files = {
            ...
            "final_analysis": final_analysis_file if final_analysis_file.exists() else None,
        ...
        if available_files["final_analysis"]:
            analysis["advanced_insights"] = self._generate_advanced_insights(analysis_data)
        
        # Add summarization analysis if available
        if available_files["summarized"]:
            analysis["summarization_analysis"] = self._analyze_summarization_results(analysis_data)
        
        return analysis

    def _load_and_validate_data(self, available_files: Dict[str, Optional[Path]]) -> Dict[str, pd.DataFrame]:
        """Load and validate all available data files."""
        data = {}
        
        # Load categorized data (required)
        categorized_df = pd.read_csv(available_files["categorized"], sep=';', encoding='utf-8-sig')
        ...
        if available_files["final_analysis"]:
            final_df = pd.read_csv(available_files["final_analysis"], sep=';', encoding='utf-8-sig')
        ...
        if available_files["summarized"]:
            summarized_df = pd.read_csv(available_files["summarized"], sep=';', encoding='utf-8-sig')
    ...
    def _validate_data_integrity(self, data: Dict[str, pd.DataFrame]):
        ...
        if missing_columns:
            raise ValueError(f"Missing required columns in categorized data: {missing_columns}")
        ...
        if "final_analysis" in data:
            ...
            final_ids = set(final_df["ticket_id"]) if "ticket_id" in final_df.columns else set()
            
            if final_ids:
                overlap = len(categorized_ids & final_ids)
                logger.info(f"Ticket ID overlap between files: {overlap:,} tickets")

    def _generate_metadata(self, available_files: Dict[str, Optional[Path]], data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        return {
            ...
            "files_analyzed": {
                "categorized_tickets": str(available_files["categorized"]) if available_files["categorized"] else None,
                "final_analysis": str(available_files["final_analysis"]) if available_files["final_analysis"] else None,
                "summarized_tickets": str(available_files["summarized"]) if available_files["summarized"] else None,
            },
            "data_summary": {
                "total_tickets": len(categorized_df),
                "unique_categories": categorized_df["categoria"].nunique(),
                "files_processed": len([f for f in available_files.values() if f is not None]),
            }
        }
    ...
    def _analyze_categories(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        return {
            ...
            "category_quality_metrics": {
                ...
                "concentration_ratio": self._calculate_concentration_ratio(category_counts),
                "dominant_category_share": round((category_counts.iloc[0] / total_tickets) * 100, 2),
            }
        }
    ...
    def _calculate_category_diversity(self, category_counts: pd.Series) -> float:
        ...
        shannon_entropy = -np.sum(proportions * np.log(proportions + 1e-10))  # Add small epsilon to avoid log(0)
    ...
    def _generate_statistical_summary(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        return {
            ...
            "distribution_analysis": {
                ...
                "categories_with_2_5_tickets": int(((category_counts >= 2) & (category_counts <= 5)).sum()),
                "categories_with_6_20_tickets": int(((category_counts >= 6) & (category_counts <= 20)).sum()),
                "categories_with_21_100_tickets": int(((category_counts >= 21) & (category_counts <= 100)).sum()),
                "categories_with_100plus_tickets": int((category_counts > 100).sum()),
            }
        }

    def _calculate_quality_metrics(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        return {
            ...
            "category_naming_quality": {
                ...
                "shortest_category_name": min([len(str(cat)) for cat in category_names]) if len(category_names) > 0 else 0,
                "longest_category_name": max([len(str(cat)) for cat in category_names]) if len(category_names) > 0 else 0,
            }
        }

    def _analyze_pipeline_performance(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        return {
            "processing_efficiency": {
                ...
                "pipeline_components_used": self._identify_pipeline_components_used(data),
            },
    ...
    def _identify_pipeline_components_used(self, data: Dict[str, pd.DataFrame]) -> List[str]:
        """Identify which pipeline components were used based on available data."""
        components = ["TicketCategorizer"]  # Always present since categorized data exists
    ...
    def _generate_advanced_insights(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    def _analyze_bullet_coverage(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for bullets_json in bullets_series:
            try:
                bullets_data = json.loads(bullets_json) if isinstance(bullets_json, str) else bullets_json
    ...
    def _analyze_summarization_results(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    def _assess_summary_quality(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if "bullets" in df.columns:
            bullets_series = df["bullets"].dropna()
            quality_metrics["bullet_completion_rate"] = round((len(bullets_series) / len(df)) * 100, 2)
    ...
    def export_comprehensive_reports(
        ...
        try:
            if export_excel:
                excel_file = self._export_excel_report(analysis, categorized_df, filename_base)
                exported_files["excel"].append(excel_file)
            
            if export_csv:
                csv_files = self._export_csv_reports(analysis, categorized_df, filename_base)
            ...
            logger.info(f"Successfully exported {sum(len(files) for files in exported_files.values())} report files")
    ...
    def _export_excel_report(self, analysis: Dict[str, Any], categorized_df: pd.DataFrame, filename_base: str) -> Path:
        ...
        with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
            ...
            # Category distribution
            category_data = []
            for category, info in analysis["category_analysis"]["category_distribution"].items():
            ...
            categorized_df.to_excel(writer, sheet_name='All_Categorized_Tickets', index=False)
            
            # Quality metrics
            quality_data = self._prepare_quality_metrics_data(analysis)
            pd.DataFrame(quality_data).to_excel(writer, sheet_name='Quality_Metrics', index=False)
            
            # Pipeline performance
            performance_data = self._prepare_performance_data(analysis)
            pd.DataFrame(performance_data).to_excel(writer, sheet_name='Pipeline_Performance', index=False)
        
        logger.info(f"Excel report exported: {excel_file}")
        return excel_file

    def _export_csv_reports(self, analysis: Dict[str, Any], categorized_df: pd.DataFrame, filename_base: str) -> List[Path]:
    ...
    def _export_text_report(self, analysis: Dict[str, Any], filename_base: str) -> Path:
        ...
        with open(text_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Total Tickets Analyzed: {metadata['data_summary']['total_tickets']:,}\n")
            f.write(f"Unique Categories Found: {metadata['data_summary']['unique_categories']}\n\n")
            ...
            f.write(f"Mean Tickets per Category: {stats['category_statistics']['mean_tickets_per_category']}\n")
            f.write(f"Median Tickets per Category: {stats['category_statistics']['median_tickets_per_category']}\n\n")
            
            # Top categories
            f.write("TOP 10 CATEGORIES:\n")
            f.write("-" * 50 + "\n")
            for i, category_info in enumerate(analysis["category_analysis"]["top_categories"], 1):
                f.write(f"{i:2d}. {category_info['category']:<35} {category_info['count']:>8,} ({category_info['percentage']:>5.1f}%)\n")
            
            # Quality metrics
            quality = analysis["quality_metrics"]
            f.write(f"\nQUALITY METRICS:\n")
            f.write("-" * 40 + "\n")
            f.write(f"Data Completeness: {quality['data_completeness']['completeness_rate']}%\n")
            f.write(f"Valid Categorizations: {quality['data_completeness']['valid_categorizations']:,}\n")
    ...
    def _prepare_summary_data(self, analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {"Metric": "Total Tickets", "Value": metadata["data_summary"]["total_tickets"]},
            {"Metric": "Unique Categories", "Value": stats["unique_categories"]},
            {"Metric": "Mean Tickets per Category", "Value": stats["category_statistics"]["mean_tickets_per_category"]},
            {"Metric": "Data Completeness Rate", "Value": f"{quality['data_completeness']['completeness_rate']}%"},
            {"Metric": "Pipeline Components Used", "Value": len(analysis["pipeline_performance"]["processing_efficiency"]["pipeline_components_used"])},
        ]

    def _prepare_quality_metrics_data(self, analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Prepare quality metrics data for export."""
        quality = analysis["quality_metrics"]
        
        return [
            {"Metric": "Total Tickets", "Value": quality["data_completeness"]["total_tickets"]},
            {"Metric": "Valid Categorizations", "Value": quality["data_completeness"]["valid_categorizations"]},
            {"Metric": "Empty Categorizations", "Value": quality["data_completeness"]["empty_categorizations"]},
            {"Metric": "Completeness Rate", "Value": f"{quality['data_completeness']['completeness_rate']}%"},
            {"Metric": "Avg Category Name Length", "Value": quality["category_naming_quality"]["average_category_name_length"]},
        ]

    def _prepare_performance_data(self, analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Prepare performance data for export."""
        performance = analysis["pipeline_performance"]
        
        return [
            {"Metric": "Total Tickets Processed", "Value": performance["processing_efficiency"]["total_tickets_processed"]},
            {"Metric": "Pipeline Components", "Value": ", ".join(performance["processing_efficiency"]["pipeline_components_used"])},
            {"Metric": "Cache Enabled", "Value": performance["cache_performance"].get("cache_enabled", "Unknown")},
        ]
    ...
    def process_pipeline_results(
        ...
        try:
            ...
            result = {
                ...
                "total_tickets": len(categorized_df),
                "total_categories": analysis["statistical_summary"]["unique_categories"],
            ...
            logger.info(f"Pipeline analysis completed successfully. Processed {result['total_tickets']:,} tickets.")
            return result
